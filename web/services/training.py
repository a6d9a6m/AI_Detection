...
from paths import (
    ...
    CHECKPOINTS as CHECKPOINTS_DIR,
    FEATURES as FEATURES_DIR,
    FEATURE_EXTRACTION_CHECKPOINTS as FEATURE_EXTRACTION_CHECKPOINTS_DIR,
    model_path as get_model_path,
    feature_path as get_feature_path,
...
try:
    ...
    from predict import evaluate as test_model
...
class TrainingManager:
    ...
    def _run_training(self, db_task_id: int, user_id: int,
        ...
        try:
            ...
            # 模型保存路径
            model_path = get_model_path(machine_type, task_id)
            checkpoint_dir = CHECKPOINTS_DIR  # 添加这行定义checkpoint_dir

            # 创建args对象用于训练
            class TrainingArgs:
                def __init__(self):
                    self.data_dir = train_dir
                    self.train_dir = train_dir  # 添加train_dir属性
...
class FeatureExtractionManager:
    ...
    def extract_features(self, voiceprint_id: int, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_path = get_feature_path(feature_filename)
    ...
    def batch_extract_features(self, voiceprint_ids: list,
        ...
        # 使用统一的路径配置
        os.makedirs(FEATURE_EXTRACTION_CHECKPOINTS_DIR, exist_ok=True)
